...
7. **Telas e fichas** — `render()` chama `renderPainel/Lancar/Mes/Contas/Futuro/Meta/Analises/Ajustes`; fichas com `sheet()`: `openTx`, `openManual`, `openAcct` (nome, tipo, excluir), `openSaldoInicial`, `openCorrigirSaldo`, `openPay`, `openTransfer`, `openCard`, `openCat`, `openEcat`, `openFix`, `openKw`, `openRestore`, `openGoal`, `openDeposit`.
8. **Método 50-30-20, ler documento, análises e dívidas**
   ...
   - Ler documento: `htmlLeitorDoc(onde)`, `montarLeitorDoc`, `lerDoc`, `mostrarLidos`, `rascunhosDoc`, `pareceRepetido`, `openLinhaBoleto`, `docNaAbaErrada`, `conferenciaEmOutraAba` + `avisoOuConferencia` (a conferência fica na aba em que foi aberta, `S.draftsAba`).
...
- `leitor.js` — leitor de documentos, 100% no aparelho. PDF com texto: pdf.js 3.11.174 (cdnjs). Foto ou PDF escaneado: Tesseract.js 5.1.1 (jsdelivr), idioma `por`. Boleto: `BarcodeDetector` (formato ITF) quando existe, senão a linha digitável reconhecida no texto; aceita só código que passa nos dígitos verificadores (módulo 10/11; fator de vencimento com o ciclo que recomeçou em 22/02/2025). Expõe `window.Leitor` (`ler`, `lerLancamentos`, `lerCupom`, `lerBoleto`, `adivinharTipo`). As bibliotecas baixam na primeira vez e ficam no cache `caderneta-libs`.
  ...
  - **BB Rende Fácil** é a aplicação automática do saldo: o banco zera a conta todo dia e guarda o resto nela. O dono vê isso como dinheiro da conta corrente, então os movimentos do Rende Fácil ficam **de fora** (`aplicacoes`) e o saldo da conta no app = conta + Rende Fácil. Por isso, com Rende Fácil no extrato, `saldosDoExtrato` descarta o saldo final impresso (não conta o aplicado) e só aceita o *Saldo Anterior* negativo (se houvesse dinheiro aplicado, o banco teria coberto a conta). Conferido: jan/2026 sem o Rende Fácil soma exatamente saldo anterior + o que ficou aplicado.
  ...
  - **Onde se importa (decidido com o dono em 15/09/2026):** a fatura do cartão, o canhoto e o boleto em **Lançar** (`htmlLeitorDoc("lancar")`, tipos fatura/cupom/boleto, cartão da fatura); o extrato da conta em **Ajustes**, no topo (`htmlLeitorDoc("ajustes")`, `#docTipo` fixo em extrato, conta do extrato), porque é conferência do saldo e traz os débitos que só o banco mostra. O mês da fatura não se escolhe: vem das compras. Se o documento parece do outro tipo (`adivinharTipo`), `docNaAbaErrada` pergunta e leva para a aba certa com o mesmo arquivo. Cada aba monta o seu leitor com os mesmos ids; por isso `render()` esvazia as abas escondidas. Enquanto houver conferência aberta numa aba, a outra só mostra o aviso (`avisoOuConferencia`) e não começa outra.
...
- `months["AAAA-MM"].tx[id]` → `{d, t, v, desc, cat, method, card, acct, acct2, k, plan, i, n, total, fix, ref, to, u, fat?, cons?, emp?}`
...
- Acerto de saldo: soma a diferença em `a.opening` e guarda `{d, delta}` em `a.adjust` (últimos 20). Vem de *Corrigir saldo* (Ajustes) ou do saldo impresso no extrato. **Nunca apague lançamento por conta própria, nem acerte saldo sozinho:** duplicado e acerto sempre passam por uma pergunta ao dono (`dupHTML`, `openRepetido`, `openAcerto`).
...
- Excluir conta pede um segundo toque (*Excluir mesmo?*): o botão fica onde as outras fichas têm *Cancelar* e *Depois*. Cartões e lançamentos ligados a uma conta excluída caem na primeira conta da lista (`acctOf`).
...
- Abas: **Lançar** = fala, gasto/entrada na mão, transferência e importar fatura, canhoto ou boleto. **Contas** = só ver: onde o dinheiro está, cartões (*Pagar fatura*, *Ver fatura*) e o extrato com *Repetidos*. **Ajustes** = importar extrato da conta, conferência, *Saldo de cada conta* (*Saldo inicial*, *Corrigir saldo*, *Editar*) e depois meta, cartões, categorias, fixos e dados.
...
- **Publicado:** `caderneta-v16` (15/09/2026). O que existe está descrito acima; os commits contam a ordem.
