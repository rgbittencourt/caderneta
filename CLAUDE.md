...
4. **Leitura da fala** — `dataDaFala` entende quando ("hoje", "amanhã", "ontem", "depois de amanhã", "dia 10", "dia 5 de outubro", "10/10"): é assim que se **agenda** falando, e roda antes do valor para o número do dia não virar valor. Sem mês, "dia 10" é o próximo dia 10, mas até três dias para trás continua no mês atual. Lançamento com data à frente ganha a marca *agendado* nas listas. `textoDaFala` junta os pedaços do ditado: cada navegador entrega de um jeito, e o do dono repete a frase inteira crescendo a cada pedaço ("325", "325 pago", "325 pago para…"). Somar tudo virava uma frase gigante e um valor absurdo (R$ 325.325.325.325.325,00). Agora, se o pedaço novo começa com o que já existe, ele substitui; se repete o fim, é ignorado. `parseOne`: números por extenso, parcelas, forma de pagamento, cartão (inclusive apelidos aprendidos), valor, "pra quem", descrição, categoria (`guessCat`: a palavra que aparece primeiro na frase manda). `splitUtterance` separa vários lançamentos por ponto final.
...
- **Carteira:** em *Corrigir saldo* de uma conta do tipo carteira, o que faltar entra como **gasto em dinheiro** (categoria Outros, forma Dinheiro), não como acerto neutro — o dinheiro sacado foi gasto, só não se sabe onde. Tem o atalho *Não sobrou nada: zerar*. Sobra a mais continua como acerto.
- **Repetidos:** a ficha mostra, de cada lado, qual fatura o lançamento abate e de onde ele veio, e tem três saídas: *São diferentes*, *Apagar o primeiro* e *Apagar o segundo* — quando o banco cobra e estorna, às vezes é o primeiro que precisa sair. Na partida, `conferirPagamentosRepetidos(true)` só **avisa** (*Depois eu vejo* / *Conferir agora*): decidir pode depender de olhar o extrato ou a fatura, e o dono não pode ser obrigado a escolher na hora de abrir o app. O caminho de sempre continua em Contas → *Repetidos*.
- **Boleto:** o leitor de boleto tem *Quando pagar* — **no vencimento (agendar)**, que é o padrão, ou hoje (já paguei).
...
- **Dívidas:** `cfg.dividas[]` → `{id, nome, tipo, parcela, total, inicio, dia, juros, card, folha, fixId}`.
  ...
  - **Primeiro desconto (pedido do dono, 25/09/2026):** `dv.primeiro` (`AAAA-MM`) é o mês em que o banco começou a descontar — costuma ser meses depois do empréstimo (o dele caiu em 20/05/2026 e o desconto começou em julho). `estadoCons` usa `ymShift(dv.primeiro, jaDesc)` como primeira parcela ainda a descontar; sem ele, continua o palpite antigo (o salário seguinte ao empréstimo).
  - **Quitação** (`openQuitacao`, `mesDaParcela`, `parcelaParaQuitar`): quanto falta para quitar hoje e mês a mês (`saldoPrice`), e quanto pagar por mês para terminar num mês escolhido — `Q = S·i/(1−(1+i)^−m)` —, com o quanto é a mais que a parcela e a economia de juros. Sem juro informado, o saldo é parcela × parcelas e não há economia. Botão *Quitar antes* na ficha da dívida (Análises) e *Quitar* na lista de Lançar, que também mostra o "quitar hoje".
...
- **Publicado:** `caderneta-v33` (25/09/2026). O que existe está descrito acima; os commits contam a ordem.
